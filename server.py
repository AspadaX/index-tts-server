...
def generate_audio_task(task_id: str, speaker_uuid: str, text: str, params: dict):
...
async def register_speaker(speaker_name: str = Form(...), file: UploadFile = File(...)):
    try:
        ...
        return BaseResponse(status=StatusEnum.Success, message="Speaker registered successfully", data=None)
    except Exception as e:
...
async def generate(request: GenerateRequest, background_tasks: BackgroundTasks):
    try:
        ...
        background_tasks.add_task(generate_audio_task, task_id, request.speaker_uuid, request.text, params)
...
async def check_status(task_id: str = Query(...)):
    if task_id not in tasks:
        # Return JSON error
        return JSONResponse(
            content={"status": "Failed", "message": "Task not found", "data": None},
            status_code=404
        )
    ...
    if task["status"] == StatusEnum.Success:
        # Return the file
        return FileResponse(task["output_path"], media_type="audio/wav", filename="generated.wav")
    elif task["status"] == StatusEnum.Failed:
        return JSONResponse(
            content={"status": "Failed", "message": task.get("message", "Unknown error"), "data": None}
        )
    else:
        return JSONResponse(
            content={"status": "InProgress", "message": "Task is running", "data": None}
        )

@app.delete("/api/v1/delete_speakers", response_model=BaseResponse)
async def delete_speaker(speaker_uuid: str): # Usually DELETE uses query param or path param, but spec says input: speaker uuid. Can be body or query. I'll use query for simplicity.
    try:
        speakers = load_speakers()
